...
from honeytrap.alerts.templates import render_discord


class DiscordChannel(AlertChannel):
    """Post alerts to a Discord webhook URL."""
    ...
    async def _send(self, alert: Alert) -> None:
        """Render the alert as a Discord embed and POST it."""
        payload = render_discord(alert)
        response = await post_json(self.webhook_url, payload)
        if response.status >= 400:
            raise RuntimeError(f"Discord webhook returned {response.status}: {response.body[:200]}")
